...
**Registration validation:** `UserCreate` (`app/schemas/auth.py`) normalizes then validates every register field — notably the CR tag is uppercased with `O`→`0` and `#` prepended, because settlement matches opponents by exact tag equality against the CR API's `#UPPERCASE` tags, so an unnormalized tag registers fine but can never settle. `frontend/src/validation.js` is a hand-kept mirror of those rules *and their error messages* (run on field blur and before submit in `AuthForm.jsx`); change both together. The backend is authoritative. `api.js::formatDetail` turns FastAPI's 422 `detail` list into readable lines.
