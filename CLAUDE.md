...
1. `POST /matchmaking/queue` creates a `MatchmakingRequest` (status `WAITING`). `stake_amount` must be one of the fixed `STAKE_TIERS` (1, 5, 10, 20, 50; `app/schemas/matchmaking.py`) — anything else is rejected with 422, and since tiers are fixed, matching on exact `stake_amount` equality also means same-tier-only matching. If another `WAITING` request exists at the same `stake_amount` (and that opponent still has sufficient balance), the two requests are atomically matched into a `Wager` (status `MATCHED`), both requests flip to `MATCHED`, and a `WAGER_LOCK` transaction (negative amount) is created for each player. The opponent match uses `.with_for_update()` to avoid two simultaneous queuers double-matching the same request. The frontend (`Matchmaking.jsx`) surfaces this as a row of tier buttons, greyed out when the live balance (threaded down via `balanceRefreshKey`) can't cover them, rather than a free-text amount field.
2. Both players are notified: a WebSocket push (`connection_manager`) to whoever was already connected, plus an SMS via Twilio (`sms_service`) with the opponent's Supercell friend link — but only to players who opted in (`User.sms_consent`) and have a `phone_number` on file, since both are now optional at registration; the opponent's friend link is always available in-app regardless via the WS/HTTP status payload.
...
**Registration validation:** `UserCreate` (`app/schemas/auth.py`) normalizes then validates every register field — notably the CR tag is uppercased with `O`→`0` and `#` prepended, because settlement matches opponents by exact tag equality against the CR API's `#UPPERCASE` tags, so an unnormalized tag registers fine but can never settle. For the same reason, `register` (`app/routers/auth.py`) also confirms the tag exists via `cr_api_service.player_exists` (404 → 422; any other CR API failure → 503, failing closed) — this server-side check has no frontend mirror. `frontend/src/validation.js` is a hand-kept mirror of those rules *and their error messages* (run on field blur and before submit in `AuthForm.jsx`); change both together. The backend is authoritative. `api.js::formatDetail` turns FastAPI's 422 `detail` list into readable lines.
...
`scripts/deploy.sh` is diff-driven: it fast-forwards to `origin/main`, then only runs `poetry install` if `pyproject.toml`/`poetry.lock` changed, only runs `alembic upgrade head` + restarts `karmine-backend.service` if backend-relevant paths changed, and only runs `npm ci && npm run build` if `frontend/` changed (the frontend systemd service serves `dist/` directly, no restart needed). When changing deploy behavior, this script is the source of truth, not the workflow file. The deploy runs in the same checkout used for development on the Pi (`/home/dindin23/PersonalProjects/Karmine`), so `deploy.sh` refuses (exit 1) unless that checkout is on `main` with no uncommitted changes to tracked files — leave it on `main` after branch work, or the next deploy fails.

`systemd/*.service` are reference copies of the two units actually installed at `/etc/systemd/system/` on the Pi (`karmine-backend`, `karmine-frontend`). They exist for history/disaster-recovery only — `deploy.sh` never reads them, so an edit here doesn't take effect until manually copied over and `daemon-reload`d on the Pi (see `systemd/README.md`). The frontend unit intentionally omits `serve`'s `-s`/single flag: the app has no client-side router, and `-s` combined with `serve`'s clean-URL redirects was silently serving `index.html` for the standalone static pages under `frontend/public/` (e.g. `/privacy.html` → login screen instead of the real page) — don't reintroduce it.
